...
<!-- Seed for DESIGN.md (iOS), macOS-DESIGN.md, tvOS-DESIGN.md,
     WEB-DESIGN.md, or ANDROID-DESIGN.md. Create once the platform
     passes ~5 views. Invoke `binding-design-doc-discipline` for the
     workflow. For macOS, also consult `macos-platform-patterns` and
     structure the doc around: navigation shell (NavigationSplitView),
     window/scene model, player-as-window-root, the ImagePipeline, and
     the sandbox/submission requirements.
